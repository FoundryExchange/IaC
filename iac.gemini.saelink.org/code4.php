...
try {
    if ($search !== "") {
        ...
        // 如果没有提供搜索关键词，仍然返回全部结果，但是也按时间戳逆序排列
        $stmt = $conn->prepare("
            SELECT c.*, a.Application_Name, p.Project_Name, p.Project_ID, a.Application_ID
            FROM Ragdoll_IaC_Code c
            LEFT JOIN Ragdoll_IaC_Application a ON c.Application_ID = a.Application_ID
...
<body>
    ...
    <table>
        ...
        foreach ($results as $row) {
            // 假设这里是正确获取到的 Project_ID 和 Application_ID
            ...
            echo "<td class='content-col'>" . htmlspecialchars($row['Code_Content']) . "</td>";
